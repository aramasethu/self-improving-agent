"""
Status Quo Data Migration Agent (Baseline)
==========================================
The original agent before self-improvement. No LangSmith, no rules,
no retries, no tool-calling. Straightforward pipeline:

  analyze_csv → map_columns → generate_code → execute_code → validate_output

Usage:
  uv run agent_status_quo.py                          # run all 100 CSVs
  uv run agent_status_quo.py data/test/test_019.csv   # run one CSV
"""
...
# ---------------------------------------------------------------------------
# LLM setup - Claude Haiku (no tracing, no LangSmith)
# ---------------------------------------------------------------------------
llm = ChatAnthropic(
...
# ---------------------------------------------------------------------------
# Build the LangGraph (simple linear pipeline — no retries, no branching)
# ---------------------------------------------------------------------------
def build_graph():
